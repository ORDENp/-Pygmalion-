...
function calculateBurnAt(createdAt) {
    ...
    // Импульсные У.Е. (20:00-23:59) → burnAt = послезавтра 00:00 (+2 дня)
    // Активные У.Е. (04:00-19:55) и ночные (00:00-03:59) → burnAt = завтра 00:00 (+1 день)
    const daysToAdd = (emissionHour >= 20) ? 2 : 1;

    const burnDate = new Date(emissionTime.getFullYear(), emissionTime.getMonth(), emissionTime.getDate() + daysToAdd, 0, 0, 0, 0);
...
function guardRequireOK() {
    ...
    if (!okKey || okKey.trim().length < 3) {
        alert('⚠️ Необходим О.К. — пройдите Порог для получения числового следа');
        return false;
    }
    return true;
}
...
function renderTransactionsRegistry(container) {
    ...
    // Генерация цепочки записей (v0.3.19 — канонический формат + .chronicle-row + кнопка спора)
    const chain = sortedTxs.map((tx, idx) => {
        const trackNum = total - idx;  // 1 внизу, N наверху
        const record = buildCanonicalRecord(tx, trackNum);
        // Индекс в исходном массиве (для открытия спора)
        const txIndex = txs.indexOf(tx);
        return `<div class="chronicle-row">
            <div class="chronicle-content">${record}</div>
            <button class="btn-dispute" data-tx-index="${txIndex}" title="Открыть спор — копия в wek.o/o0/">⚠️ Открыть спор</button>
        </div>`;
    }).join('');
    ...
    // Навешиваем обработчики кнопок спора
...
/**
 * Открыть спор по транзакции → копия в wek.o/o0/ (v0.3.25)
 * Генерирует ссылку: wek.o/o0/#<track>.<hhmm>:<ddmmyy>.::<fromOK>::.№<ue>.::<disputeCode>::."—".(срок).(оценка).(дата выкупа).(О.К.-3)
 */
function openDispute(txIndex) {
    ...
    const disputeLink = `wek.o/o0/#${trackNum}.${hhmm}:${ddmmyy}.::${fromOK}::.${ueFormatted}.::${disputeCode}::."—".${delivery}.${valuation}.${buyout}.${ok3}`;
...
function joinOrder(orderId) {
    ...
    if (orderId === '01') {
        ...
        // Подтверждение (v0.3.25 — явный диалог)
        const confirmed = confirm('Вы хотите подать заявку предстоятелю ОрДена ::01:: Хранители?\n\nДа — подать заявку\nНет — отмена');
        if (!confirmed) return;
...
function renderOrderChatMessages(orderId) {
    ...
    const msgs = loadOrderChats(orderId);
    if (msgs.length === 0) {
        el.innerHTML = '<p class="order-chat-empty">Сообщений пока нет</p>';
        return;
    }
...
function renderDeptChatMessages(deptId) {
    ...
    const msgs = loadDeptChats(deptId);
    if (msgs.length === 0) {
        el.innerHTML = '<p class="dept-chat-empty">Сообщений пока нет</p>';
        return;
    }
